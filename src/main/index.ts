...
// 导入所有图标
const resourceRoot = app.isPackaged ? app.getAppPath() : process.cwd();
const iconPath = join(resourceRoot, 'resources');
...
if (!isSingleInstance) {
  ...
  app.whenReady().then(() => {
    ...
    if (process.platform === 'darwin') {
      const dockIcon = nativeImage.createFromPath(join(iconPath, 'icon.icns'));
      if (!dockIcon.isEmpty()) {
